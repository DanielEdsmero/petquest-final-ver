...
/*
 * React Bits — MagicRings (https://reactbits.dev). MIT.
 * Concentric rings bloom outward on a WebGL2 shader quad.
 *
 * The GLSL below is upstream's, byte for byte.
 *
 * This is the only thing in the app that pulls in three.js, so it is always
 * loaded through React.lazy — see CelebrationRings.jsx. Import it directly and
 * three lands in the main bundle.
 *
 * [petquest] Adaptations:
 *   - Reduced motion renders nothing. Upstream already bails cleanly when
 *     WebGL2 is unavailable, so the surrounding overlay must not depend on
 *     this drawing anything, and it doesn't.
 */
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    if (!mount || reduceMotion) return; // [petquest]
